    ...
    if (thread_id < height) // each thread represents an image row
    {
        ...
        for (int w = 0; w < width - 1; w++)
        {
            ...
            else if (!a[thread_id * width + w] && interval_found)
            {
                interval_found = 0;
                temp = interval_row[thread_id];
                ir[thread_id * width + temp].x2 = w - 1;
                temp++;
                interval_row[thread_id] = temp;
            }
        }
        ...
        if (!a[thread_id * width + width - 1])
        {
            ...
            {
                temp = interval_row[thread_id];
                ir[thread_id * width + temp].x2 = width - 2;
                temp++;
                interval_row[thread_id] = temp;
            }
        }
        else
        {
            ...
            {
                temp = interval_row[thread_id];
                ir[thread_id * width + temp].x1 = width - 1;
            }
            ir[thread_id * width + temp].x2 = width - 1;
            temp++;
            interval_row[thread_id] = temp;
        }
    }
...
__global__ void pibr_block_creation(introw_t *ir, int height, int width, blocktype *block, int *interval_row, int *is_block, int *blockno)
{
    ...
    if (thread_id < height) // each thread represents an image row
    {
        ...
        if (interval_row[(thread_id - 1)] > 0) // if previous row has interval
        {
            ...
            {
                is_block[(thread_id)*width + i] = 1;
                for (int j = j_curr; j < interval_row[(thread_id)-1]; j++) // for every interval in previous row (interval_row = number of intervals)
                {
                    j_prev = j;
                    if (ir[(thread_id)*width + i].x1 < ir[(thread_id - 1) * width + j].x1)
                    {
                        break;
                    }
                    else if (ir[(thread_id)*width + i].x1 == ir[(thread_id - 1) * width + j].x1 && ir[(thread_id)*width + i].x2 == ir[(thread_id - 1) * width + j].x2)
                    {
                ...
                {
                    block[(thread_id)*width + blockno[thread_id]].x1 = ir[(thread_id)*width + i].x1;
                    block[(thread_id)*width + blockno[thread_id]].x2 = ir[(thread_id)*width + i].x2;
                    block[(thread_id)*width + blockno[thread_id]].y1 = thread_id;
                    block[(thread_id)*width + blockno[thread_id]].y2 = thread_id;
                    blockno[thread_id]++;
                }
    ...
    if (thread_id < height) // each thread represents an image row
    {
        for (int i = 0; i < blockno[thread_id]; i++)
        {
            ...
            for (int k = thread_id + 1; k - temp == 1 && k < height; k++)
            {
                ...
                {
                    if (block[(thread_id)*width + i].x1 == ir[(k)*width + jjj].x1 && block[(thread_id)*width + i].x2 == ir[(k)*width + jjj].x2)
                    {
                        temp = k;
                        break;
                    }
                    else if (block[(thread_id)*width + i].x1 < ir[(k)*width + jjj].x1)
                    {
            ...
            block[(thread_id)*width + i].y2 = temp;
        }
    }
}
