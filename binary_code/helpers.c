...
void cheby_rebuild(int height, int width, int *dim_order, float *reb_img, float *tt, float *tb)
{
    ...
    for (int x = 0; x < dim_order[0]; x++)
    {
        for (int y = 0; y < dim_order[0]; y++)
        {
            if (reb_img[x * dim_order[0] + y] < min_IR)
                min_IR = reb_img[x * dim_order[0] + y];
            if (reb_img[x * dim_order[0] + y] > max_IR)
                max_IR = reb_img[x * dim_order[0] + y];
        }
    }
    ...
    for (int x = 0; x < dim_order[0]; x++)
    {
        for (int y = 0; y < dim_order[0]; y++)
        {
            if (reb_img[x * dim_order[0] + y] > min_IR + (max_IR - min_IR) / 2)
                reb_img[x * dim_order[0] + y] = 255;
            else
                reb_img[x * dim_order[0] + y] = 0;
        }
    }
}
...
double nire_calculation(unsigned char *original_image, float *reconstructed_image, int height, int width)
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            if (original_image[i * height + j] == 255)
                original_image[i * height + j] = 1;
